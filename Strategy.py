...
class Strategy:
    ...
    def __init__(self,settings,st_tickers_returns,indicators_dict):
        ...
        def compute_blend_factors(sources: dict, returns_df: pd.DataFrame, settings: dict) -> pd.DataFrame:
            """
            Compute normalized blend factors for each source using fixed factors.

            Parameters
            ----------
            sources      : dict  name -> (weights_df, fixed_factor)
            returns_df   : pd.DataFrame  DatetimeIndex x tickers, daily returns (unused, kept for interface consistency)
            settings     : dict  (unused, kept for interface consistency)
        ...
        # ======================================================================
        # Main portfolio weight computation block
        # ======================================================================
