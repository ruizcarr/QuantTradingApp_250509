...
class BacktestVectorized:
    ...
    def compute_backtest_until_convergence(
        ...
        for i in range(max_iterations):
            ...
            # In compute_backtest_until_convergence, before storing results:
            alpha = 0.6  # damping factor (tune between 0.5-0.8)
            if i > 0:
                damped = np.round(
                    alpha * new_pos.values + (1 - alpha) * all_positions[i],
                    0
                ).astype(np.int32)
                all_positions[i + 1] = damped
            else:
            ...
            # 1. Check for Absolute Equality (The Dream)
            if np.array_equal(all_positions[i + 1], all_positions[i]):
                print(f"✅ Full convergence reached at iteration {i}")
                break

            if i > 3:
                num_diffs = np.count_nonzero(all_positions[i + 1] != all_positions[i])
                # Detect 2-cycle oscillation
                if i > 1 and np.array_equal(all_positions[i + 1], all_positions[i - 1]):
                    print(f"2-cycle oscillation detected at iteration {i}, averaging states")
                    all_positions[i + 1] = np.round(
                        (all_positions[i + 1].astype(float) + all_positions[i].astype(float)) / 2
                    ).astype(np.int32)
                    break
                if num_diffs < 9:
                    print(f"Convergence within tolerance ({num_diffs} flips at iteration {i})")
    ...
    def compute_backtest(
        ...
        portfolio_to_invest = portfolio_value_usd.shift(1).rolling(self.settings.portfolio_window,min_periods=self.settings.min_periods).min()
        ...
        target_size = np.round(target_size_raw, 0).astype(int)

        #Long Only
        target_size=target_size.clip(0)

        #Clip max number of contracts
        target_size = target_size.clip(None,max_n_contracts)
